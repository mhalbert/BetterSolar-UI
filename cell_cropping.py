...
def extract_CellGridAndModuleCorners(CVImage, DisplayImage, NumCells_x, NumCells_y, blurring_steps=None,
    ...
    if method == "houghlines":
        ...
        while True:
            ...
            cv2.circle(image_Reduced, (int(refPt[event_num - 1, 0]), int(refPt[event_num - 1, 1])), 2, (0, 255, 0), thickness=2)
    ...
    cv2.line(image_Outline, (int(corners[0, 0]), int(corners[0, 1])), (int(corners[1, 0]), int(corners[1, 1])), (0, 0, 255), 2)
    cv2.line(image_Outline, (int(corners[1, 0]), int(corners[1, 1])), (int(corners[2, 0]), int(corners[2, 1])), (0, 0, 255), 2)
    cv2.line(image_Outline, (int(corners[2, 0]), int(corners[2, 1])), (int(corners[3, 0]), int(corners[3, 1])), (0, 0, 255), 2)
    cv2.line(image_Outline, (int(corners[3, 0]), int(corners[3, 1])), (int(corners[0, 0]), int(corners[0, 1])), (0, 0, 255), 2)
...
# Auto
def CellCropComplete(image_file, save_path, NumCells_x=12, NumCells_y=6, corners_get='auto'):
    filepath_cell_images = os.path.dirname(image_file)
