...
def scale():
    ...
    # temporary fix to deal with scale issues
    if scaling > 2:
        scaling = 1
    else:
        scaling = 1.2
...
def preview_window(output_path, files, module):
    ...
    image_layout = [
        ...
        [sg.Text(filename, size=(660, None), key='-FILENAME-', font='Sathu 11 underline', text_color='#1a385c',
                 justification='c')]
    ]
...
def results_window(module_names, model):
    ...
    layout = [
        [sg.Text('Results', size=(window_width, 1), font='Sathu 22', text_color='white', background_color=banner_color,
    ...
    while True:
        ...
        if button == '-FOLDER LIST-':
            ...
            window['-GRADE-'].update('*FAIL*' if not stats['rating'] else '*PASS*')
...
def home_page():
    ...
    file_select_layout = [
        ...
        [sg.Text('Folder:', font=font, pad=(10, 10), background_color=section_color),
         sg.InputText(size=(30, 1), enable_events=True, key='-FOLDER-', default_text=default_folder),
         sg.FolderBrowse(font=button_font, pad=(10, 10), )],
        ...
        [sg.Listbox(file_list, enable_events=True, font=listbox_font, select_mode='multiple', size=(40, 15),
                    key="-FILES LIST-", pad=(10, 1))],
        [sg.Checkbox('Select All', enable_events=True, key='-ALL-', default=False, background_color=section_color,
                     font=font, pad=(10, 1))],
    ]
    ...
    grading_criteria_table = [
        ...
        [sg.Input(default_text=default_percentages[0], justification='c', size=(5, None), pad=((1, 11), (1, 5)),
                  key='-CRACK %-', font=info_font, background_color=section_color),
         sg.Text('%', font=info_font, background_color=section_color),
         sg.Input(default_text=default_percentages[1], justification='c', size=(5, None), pad=((12, 12), (1, 5)),
                  key='-CONTACT %-', font=info_font, background_color=section_color),
         sg.Text('%', font=info_font, background_color=section_color),
         sg.Input(default_text=default_percentages[2], justification='c', size=(5, None), pad=((11, 12), (1, 5)),
                  key='-INTERCONNECT %-', font=info_font, background_color=section_color),
         sg.Text('%', font=info_font, background_color=section_color),
         sg.Input(default_text=default_percentages[3], justification='c', size=(5, None), pad=((11, 12), (1, 1)),
                  key='-CORROSION %-', font=info_font, background_color=section_color),
         sg.Text('%', font=info_font, background_color=section_color)],
        [sg.Input(default_text=default_cells[0], justification='c', size=(5, None), pad=((1, 5), (1, 5)),
                  key='-CRACK #-', font=info_font, background_color=section_color),
         sg.Text('cells', font=info_font, background_color=section_color),
         sg.Input(default_text=default_cells[1], justification='c', size=(5, None), pad=((5, 5), (1, 5)),
                  key='-CONTACT #-', font=info_font, background_color=section_color),
         sg.Text('cells', font=info_font, background_color=section_color),
         sg.Input(default_text=default_cells[2], justification='c', size=(5, None), pad=((5, 5), (1, 5)),
                  key='-INTERCONNECT #-', font=info_font, background_color=section_color),
         sg.Text('cells', font=info_font, background_color=section_color),
         sg.Input(default_text=default_cells[3], justification='c', size=(5, None), pad=((5, 5), (1, 1)),
                  key='-CORROSION #-', font=info_font, background_color=section_color),
         sg.Text('cells', font=info_font, background_color=section_color),
         ],
    ]
    ...
    grading_criteria_layout = [
        [sg.Text('Module Grading Criteria:', pad=((1, 1), (5, 1)), font=font, background_color=section_color),
         sg.Text('minimum values resulting in a module grade of "FAIL"', pad=((1, 1), (5, 1)), font=info_font,
                 background_color=section_color)],
    ...
    grading_col = sg.Column(grading_criteria_layout, background_color=section_color,
                            size=(window_width * (2 / 3), window_height / 6))
    ...
    input_display_layout = [
        ...
        [sg.Text(
            'Click "preview" to begin. Use the arrow keys to flip through your selected files. Check "Select All" to preview all images.',
            pad=(10, None), font=info_font, background_color=section_color)],
        [sg.Frame('', image_layout, size=(633, 322), pad=(10, 10), background_color='#D8D8D8',
                  element_justification='c', relief='flat'),
    ...
    file_select_col = sg.Column(file_select_layout, background_color=section_color,
                                size=(window_width / 3, window_height * 0.56), element_justification='l')
    model_select_col = sg.Column(processing_settings_layout, background_color=section_color,
                                 size=(window_width, window_height / 6), element_justification='l')
    input_display_col = sg.Column(input_display_layout, background_color=section_color,
                                  size=(window_width * 2 / 3, window_height * 0.56), element_justification='l')

    layout = [
        [sg.Text('Home', size=(window_width, 1), font='Sathu 22', text_color='white', background_color=banner_color,
                 justification='center')],
        ...
        [sg.Button('Run', size=(5, 1), font=button_font, pad=(5, 10), button_color='#cf3f32'),
         sg.Button('View Previous Results', font=button_font, pad=(5, 10))],
        [sg.Button('Back to Menu', font=button_font, pad=(5, 1))],
        [sg.Text('', size=(window_width, 1), font='Sathu 22', text_color='white', background_color=banner_color,
                 justification='center', pad=((1, 1), (10, 1)))],
    ]
    ...
    while True:
        ...
        if event == SYMBOL_DOWN and (len(values['-FILES LIST-']) > 1 or values['-ALL-']):
            image_counter += 1
            path = values['-FOLDER-']
            ...
            window['-IMAGE-'].update(data=bio.getvalue())
        if event == SYMBOL_UP and (len(values['-FILES LIST-']) > 1 or values['-ALL-']):
            image_counter -= 1
            path = values['-FOLDER-']
            num_files = len(file_list)
            if image_counter < 0:
...
def main():
    ...
    layout = [
        ...
        [sg.Text('Better Solar, 2022', size=(500, 1), font='Sathu 10', text_color='white',
                 background_color=banner_color, justification='center', pad=((1, 1), (70, 1)))],
    ]

    window = sg.Window("Better Solar", layout, size=(700, 285), alpha_channel=1.0, grab_anywhere=True,
